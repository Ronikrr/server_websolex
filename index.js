...
// Connect to MongoDB
mongoose.connect('mongodb://127.0.0.1:27017/web_admin')
    .then(() => {
...
app.post('/users', async (req, res) => {
    try {
        ...
        const transporter = nodemailer.createTransport({
            ...
            auth: {
                user: 'ronikgorasiya@gmail.com', // Your email address
                pass: 'Ramesh0@1971@0Varsha',  // Your email password or application-specific password
            },
        });

        // Set up email data with email and password
        const mailOptions = {
            from: 'ronikgorasiya@gmail.com',  // Sender's email
            to: email,                     // Recipient's email (the one entered by the user)
            subject: 'Registration Successful', // Subject of the email
            text: `Hi ${name},\n\nThank you for registering with us! Your account is currently pending approval.\n\nHere are your login details:\n\nEmail: ${email}\nPassword: ${password}\n\nBest regards,\nYour Company`, // Email body including email and password
        };

        // Send the email
        await transporter.sendMail(mailOptions);

        // Send the response with the user's details including the entered email
        res.status(201).json({
        ...
        // Check if the error is due to a duplicate username/email (code 11000 is MongoDB's duplicate key error code)
        if (error.code === 11000) {
            return res.status(400).json({ message: "Username or email already exists." });
        }
...
app.put('/api/contactdetails', async (req, res) => {
    ...
    try {
        const updates = req.body;
        const updatedcontactdetails = await contactdetails.findByIdAndUpdate( updates, { new: true });
        if (!updatedcontactdetails) {
            return res.status(404).json({ message: 'Team clients not found' });
        }
        res.status(200).json({ message: 'Team clients updated successfully', member: updatedcontactdetails });
    } catch (error) {
        console.error('Error updating team member:', error);
        res.status(500).json({ message: 'Internal Server Error' });
    }
...
app.listen(8000,'192.168.29.189', () => {
